...
- **身分驗證成功後會記住在這台裝置上**,同一次瀏覽(甚至下次重開網頁)都不用再輸入。如果別人借用
  你的手機/電腦測試,不小心輸入了別的同事的姓名+密碼,之後這台裝置就會一直被當成那位同事操作。
  工具列右上角會顯示目前這台裝置記住的身分(例如「身分:王小明」),**點一下就能登出**,登出後下一次
  編輯或勾選會重新要求輸入姓名+密碼。公用裝置(例如公佈欄旁邊的平板)建議每次用完都手動登出
