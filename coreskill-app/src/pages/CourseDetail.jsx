...
export default function CourseDetail() {
  ...
  const { currentUser, courses, getEnrollmentForUser, enrollInCourse, updateEnrollment, getCourseDiscussions, addDiscussion } = useAuth();
  ...
  // Video progress interval - only when playing
  useEffect(() => {
    if (isPlaying) {
  ...
  if (!course) {
    return (
      <div className="p-8 text-center">
        ...
        <button onClick={() => navigate('/courses')} className="mt-4 text-blue-600 hover:text-blue-700 text-sm">
          返回課程庫
        </button>
      </div>
    );
  ...
  const allSubmitted = enrollment?.reportSubmitted && enrollment?.surveySubmitted && enrollment?.quizSubmitted;
  ...
  const progressItems = [
    ...
    {
      label: '心得報告',
      done: enrollment?.reportSubmitted,
      value: enrollment?.reportSubmitted ? '已提交' : '未提交',
    },
    {
      label: '滿意度調查',
      done: enrollment?.surveySubmitted,
      value: enrollment?.surveySubmitted ? '已提交' : '未提交',
    },
    {
      label: '課程測驗',
      done: enrollment?.quizSubmitted,
      value: enrollment?.quizSubmitted ? `${enrollment.quizScore}分` : '未完成',
    },
    {
      label: '審核狀態',
      done: enrollment?.reviewStatus === 'approved',
      value: { approved: '已通過', pending: '審核中', rejected: '已退回', null: '-' }[enrollment?.reviewStatus] || '-',
      special: enrollment?.reviewStatus === 'rejected' ? 'rejected' : null,
    },
    {
      label: '結業證書',
      done: enrollment?.certificateIssued,
      value: enrollment?.certificateIssued ? '已發放' : '未發放',
    },
  ];
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div className="p-4 flex gap-3 flex-wrap">
              {!isEnrolled ? (
                ...
                <>
                  <button
                    onClick={() => navigate(`/courses/${id}/submit`)}
                    disabled={!canSubmit}
                    ...
                    {[
                      ...
                      <div
                        ...
                        className={`flex items-center gap-3 p-3 rounded-lg ${
                          progress > (idx + 1) * 20 ? 'bg-green-50' : 'bg-gray-50'
                        }`}
                      >
                        {progress > (idx + 1) * 20 ? (
                        ...
                        {progressItems.map((item) => (
                          ...
                          >
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <p className={`text-sm font-medium ${item.done ? 'text-green-800' : 'text-gray-700'}`}>
                                  {item.label}
                                </p>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            ...
            <div className="space-y-2 text-sm">
              {[
                ...
                { label: `通過測驗（${course.passingScore}分）`, done: enrollment?.quizSubmitted && enrollment?.quizScore >= course.passingScore },
