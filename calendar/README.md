...
- **編輯密碼是共用的**:所有能儲存的人共用同一組密碼,無法真正驗證身分 —— 頁面上的「異動紀錄」只是
  請每台裝置第一次儲存時自報姓名,存在該裝置本機記住,之後每次存檔都會帶上這個名字並記錄在試算表的
  `AuditLog` 分頁(時間、部門月份、哪一天、姓名、改了什麼)。這是「方便追蹤」而不是「身分驗證」,
  有心人還是可以亂填姓名,不能當作正式的稽核依據
