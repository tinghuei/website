...
function StarRating({ value, onChange, label }) {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      {value > 0 && (
        <span className="text-sm text-gray-500">{STAR_LABELS[value]}</span>
      )}
    </div>
  );
}
...
export default function SubmitReport() {
  ...
  const { currentUser, courses, getEnrollmentForUser, submitReport, checkAndSetPendingReview } = useAuth();
  ...
  const [ratings, setRatings] = useState(
    enrollment?.surveyData
      ? {
          content: enrollment.surveyData.content || 0,
          instructor: enrollment.surveyData.instructor || 0,
          materials: enrollment.surveyData.materials || 0,
          practical: enrollment.surveyData.practical || 0,
        }
      : { content: 0, instructor: 0, materials: 0, practical: 0 }
  );
  const [overall, setOverall] = useState(enrollment?.surveyData?.overall || 0);
  const [suggestions, setSuggestions] = useState(enrollment?.surveyData?.suggestions || '');
  ...
  if (!course || !enrollment) {
    return (
      <div className="p-8 text-center">
        ...
        <button onClick={() => navigate('/courses')} className="mt-3 text-blue-600 text-sm hover:text-blue-700">
          返回課程庫
        </button>
      </div>
    );
          ...
          {activeTab === 'report' && (
            <div>
              ...
              <p className="text-sm text-gray-500 mb-4">
                請描述您在本課程中的學習心得、收穫及如何應用於實際工作中。（最少 {reportMinLength} 字）
              </p>
        ...
        <div className="px-6 py-4 bg-gray-50 rounded-b-xl flex items-center justify-between gap-4">
          ...
          <button
            ...
            className={`px-6 py-2.5 rounded-lg font-medium text-sm transition-colors flex items-center gap-2 ${
              reportValid && surveyValid && !loading
