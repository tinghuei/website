...
- **編輯密碼是共用的**:所有能儲存的人共用同一組密碼,無法真正驗證身分 —— 每台裝置第一次儲存時會
  請你自報姓名,存在該裝置本機記住,之後每次存檔都會帶上這個名字,記錄在試算表的 `AuditLog` 分頁
  (時間、部門月份、哪一天、姓名、改了什麼)。網頁上沒有顯示這份紀錄的畫面,要查誰改了什麼,直接
  打開 Google 試算表看 `AuditLog` 分頁即可。這是「方便追蹤」而不是「身分驗證」,有心人還是可以
  亂填姓名,不能當作正式的稽核依據
